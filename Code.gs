...
//  File: Code.gs   (paste this in Google Apps Script editor)
// ============================================================
...
function doGet(e) {
  return ContentService
    .createTextOutput(JSON.stringify({ success: true, message: 'Temple API running' }))
    .setMimeType(ContentService.MimeType.JSON);
}
...
function handleAction(p) {
  switch (p.action) {
    ...
    default: return { success: false, message: 'Unknown action: ' + p.action };
  }
}
...
function doLogin(p) {
  ...
  return {
    ...
    role: user.Role,
    permissions: {
      HomeEdit: perms.HomeEdit || 'No',
      Finance: perms.Finance || 'No',
      Reports: perms.Reports || 'No',
      Downloads: perms.Downloads || 'No'
    }
  };
}
...
function getNextReceipt() {
  ...
  return { receiptNo: prefix + String(num).padStart(6, '0') };
}
...
function saveTransaction(p) {
  ...
  sheet.appendRow([
    p.ReceiptNo, p.Date, p.Name, p.Area, p.Category,
    p.Amount, p.PaymentType, p.WhatsApp, p.Remarks, p.EnteredBy,
    new Date()
  ]);
  return { success: true };
}
...
function getReport(type) {
  ...
  if (type === 'daily') {
    data = sumByKey(income, 'Date');
    columns = ['Date', 'Total', 'Count'];
  } else if (type === 'monthly') {
    const byMonth = {};
    income.forEach(t => {
      const m = (t.Date||'').slice(0,7);
      byMonth[m] = (byMonth[m]||0) + (parseFloat(t.Amount)||0);
    });
    data = Object.entries(byMonth).map(([m,v]) => ({ Month: m, Total: '₹'+v.toLocaleString('en-IN') }));
    columns = ['Month','Total'];
  } else if (type === 'yearly') {
    const byYear = {};
    income.forEach(t => {
      const y = (t.Date||'').slice(0,4);
      byYear[y] = (byYear[y]||0) + (parseFloat(t.Amount)||0);
    });
    data = Object.entries(byYear).map(([y,v]) => ({ Year: y, Total: '₹'+v.toLocaleString('en-IN') }));
    columns = ['Year','Total'];
  } else if (type === 'category') {
    data = sumByKey(income, 'Category');
    columns = ['Category','Total','Count'];
  } else if (type === 'payment') {
    data = sumByKey(income, 'PaymentType');
    columns = ['PaymentType','Total','Count'];
  } else if (type === 'area') {
    data = sumByKey(income, 'Area');
    columns = ['Area','Total','Count'];
  } else if (type === 'user') {
    data = sumByKey(income, 'EnteredBy');
    columns = ['EnteredBy','Total','Count'];
  } else if (type === 'balance') {
    const totalIncome = income.reduce((s,t) => s+(parseFloat(t.Amount)||0),0);
    const totalExpense = txns.filter(t=>t.Category==='Expenses').reduce((s,t)=>s+(parseFloat(t.Amount)||0),0);
    data = [
      { Type:'Total Income', Amount:'₹'+totalIncome.toLocaleString('en-IN') },
      { Type:'Total Expense', Amount:'₹'+totalExpense.toLocaleString('en-IN') },
      { Type:'Balance', Amount:'₹'+(totalIncome-totalExpense).toLocaleString('en-IN') }
    ];
    columns = ['Type','Amount'];
  }
...
// ===== USERS =====
function getUsers() {
  const users = sheetToObjects('Users').map(u => ({ Username: u.Username, Role: u.Role, Status: u.Status }));
...
function deleteUser(p) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][uCol] === p.username) { sheet.deleteRow(i+1); return { success: true }; }
  }
...
function deleteMasterItem(p) {
  ...
  for (let i = 0; i < data.length; i++) {
    if (data[i][0] === p.value) { sheet.deleteRow(i+1); return { success: true }; }
  }
...
function getPermissions() {
  return { success: true, permissions: sheetToObjects('Permissions') };
}
function savePermission(p) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][uCol] === p.Username) {
      ['HomeEdit','Finance','Reports','Downloads'].forEach(k => {
        const col = headers.indexOf(k);
        if (col >= 0) sheet.getRange(i+1, col+1).setValue(p[k] || 'No');
      });
      return { success: true };
    }
  }
...
function getSettings() {
  return { success: true, settings: settingsToObj() };
}
function saveSettings(p) {
  const sheet = SS.getSheetByName('Settings');
  const data = sheet.getDataRange().getValues();
  const keys = data.map(r => r[0]);
  const keys_to_save = Object.keys(p).filter(k => k !== 'action');
...
// ===== PHOTOS =====
function getPhotos() {
  const photos = sheetToObjects('Photos').map(p => ({ url: p.URL, caption: p.Caption, type: p.Type }));
