...
let allUsers = [];        // cached users
let allPermissions = {};  // { username: { homeEdit, finance, reports, downloads } }
let siteSettings = {};    // from Settings sheet
...
function saveSession() {
  sessionStorage.setItem('temple_user', JSON.stringify(currentUser));
}
function clearSession() {
  sessionStorage.removeItem('temple_user');
}
...
async function loadPublicData() {
  try {
    ...
    console.warn('Could not load public data:', e);
  }
}
...
function renderHomeData(data) {
  ...
  if (s.MapURL) {
    document.getElementById('map-container').innerHTML =
      `<iframe src="${s.MapURL}" allowfullscreen loading="lazy"></iframe>`;
  }
...
function buildSlider(images) {
  ...
  slides.forEach((img, i) => {
    ...
    div.innerHTML = `<div class="slide-text"><h2 id="temple-name-slide">${CONFIG.TEMPLE_NAME}</h2><p>${img.caption || CONFIG.TEMPLE_TAGLINE}</p></div>`;
...
function startSlider() {
  ...
  sliderTimer = setInterval(() => changeSlide(1), 4000);
}
...
function renderUpdatesPage() {
  apiCall({ action: 'getPublicData' }).then(res => {
    renderUpdates(res.updates || [], 'updates-full', 0);
  }).catch(() => {});
}

// ===== FESTIVALS =====
function renderFestivals(festivals) {
  const el = document.getElementById('festivals-list');
  if (!el) return;
  el.innerHTML = '';
...
function renderPhotosPage() {
  ...
  apiCall({ action: 'getPhotos' }).then(res => {
    ...
    el.innerHTML = '<p style="padding:2rem;color:var(--muted)">Could not load photos.</p>';
  });
}
...
async function fetchCategories() {
  try { const r = await apiCall({ action: 'getCategories' }); return r.categories || []; } catch(e) { return []; }
}
async function fetchPayments() {
  try { const r = await apiCall({ action: 'getPaymentTypes' }); return r.payments || []; } catch(e) { return []; }
}
...
async function saveFinance() {
  ...
  if (!date || !name || !category || !amount || !payment) {
    showFinanceMsg('Please fill all required fields (*).', 'error');
    return;
  }

  const payload = {
    action: 'saveTransaction',
    Date: date, Name: name, Area: area, Category: category,
    Amount: amount, PaymentType: payment,
    WhatsApp: whatsapp, Remarks: remarks,
    ReceiptNo: receipt, EnteredBy: currentUser.username
  };
  ...
  try {
    ...
    if (res.success) {
      ...
      showFinanceMsg(res.message || 'Save failed.', 'error');
    }
  } catch(e) {
    showFinanceMsg('Network error. Please try again.', 'error');
  }
}

function fillReceipt(d) {
  setText('r-temple', siteSettings.TempleName || CONFIG.TEMPLE_NAME);
  setText('r-no', d.receipt);
  setText('r-date', d.date);
  setText('r-name', d.name);
  setText('r-cat', d.category);
  setText('r-amount', '₹' + fmt(d.amount));
  setText('r-pay', d.payment);
  setText('r-by', currentUser.username);
}
...
function resetFinance() {
  ['f-name','f-area','f-amount','f-whatsapp','f-remarks'].forEach(id => {
...
async function loadTransactions() {
  ...
  try {
    ...
    tbody.innerHTML = '<tr><td colspan="11" class="loading-td">Could not load transactions.</td></tr>';
  }
}
...
function fillFilterDropdowns() {
  ...
  masterPayments.forEach(p => {
    const o = document.createElement('option'); o.value = p; o.textContent = p; paySel.appendChild(o);
  });
}
...
function renderTransactionTable(data) {
  ...
  page.forEach((t, i) => {
    ...
    tbody.innerHTML += `<tr>
      ...
      <td>${t.Remarks||''}</td>
      <td>${t.EnteredBy||''}</td>
      <td>${isLast ? `<button class="btn-outline" style="padding:0.3rem 0.6rem;font-size:0.78rem" onclick="editLastTxn()">Edit</button>` : ''}</td>
    </tr>`;
  });
...
function renderPagination(total) {
  ...
  for (let i = 1; i <= pages; i++) {
    ...
    btn.textContent = i;
    btn.onclick = () => { txnPage = i; renderTransactionTable(allTransactions); };
    el.appendChild(btn);
  }
}
...
function editLastTxn() {
  showPage('finance');
  showToast('Edit mode: modify and save again.');
}
...
async function generateReport(type) {
  ...
  head.innerHTML = '<tr><td colspan="5" class="loading-td">Generating report...</td></tr>';
  body.innerHTML = '';
  try {
    ...
    head.innerHTML = '<tr><td class="loading-td">Error loading report.</td></tr>';
  }
  ...
  try {
    ...
    allUsers.forEach((u, i) => {
      tbody.innerHTML += `<tr>
        <td>${u.Username||''}</td>
        <td>${u.Role||''}</td>
        <td><span class="status-badge ${u.Status==='active'?'green':'red'}">${u.Status||''}</span></td>
        <td>
          <button class="btn-outline" style="font-size:0.78rem;padding:0.3rem 0.6rem" onclick="openUserModal(${i})">Edit</button>
          <button class="btn-outline" style="font-size:0.78rem;padding:0.3rem 0.6rem;margin-left:4px" onclick="deleteUser(${i})">Delete</button>
        </td>
      </tr>`;
    });
  } catch(e) {
    tbody.innerHTML = '<tr><td colspan="4" class="loading-td">Error loading users.</td></tr>';
  }
}
...
async function loadPermissions() {
  ...
  try {
    ...
    perms.forEach(p => {
      ...
      tbody.innerHTML += `<tr>
        <td>${p.Username}</td>
        ${['HomeEdit','Finance','Reports','Downloads'].map(key =>
          `<td><select onchange="updatePerm('${p.Username}','${key}',this.value)">
            <option value="Yes" ${p[key]==='Yes'?'selected':''}>Yes</option>
            <option value="No"  ${p[key]==='No' ?'selected':''}>No</option>
          </select></td>`
        ).join('')}
        <td><button class="btn-gold" style="font-size:0.78rem;padding:0.35rem 0.65rem" onclick="savePerm('${p.Username}')">Save</button></td>
      </tr>`;
    });
  } catch(e) {
    tbody.innerHTML = '<tr><td colspan="6" class="loading-td">Error.</td></tr>';
  }
}
function updatePerm(username, key, val) {
  if (allPermissions[username]) allPermissions[username][key] = val;
}
async function savePerm(username) {
  const p = allPermissions[username];
  try {
    const res = await apiCall({ action: 'savePermission', ...p });
    if (res.success) showToast('Permission saved!');
...
async function loadHomepageEditForm() {
  try {
    ...
    const fields = {
      'he-name': s.TempleName, 'he-tagline': s.Tagline, 'he-timings': s.Timings,
      'he-address': s.Address, 'he-phone': s.Phone, 'he-email': s.Email,
      'he-whatsapp': s.WhatsApp, 'he-mapurl': s.MapURL, 'he-logo': s.LogoURL,
      'he-upi': s.DonationUPI, 'he-qr': s.QRCodeURL,
      'he-upd1': s.Update1, 'he-upd2': s.Update2, 'he-upd3': s.Update3,
      'he-fes1': s.Festival1, 'he-fes2': s.Festival2,
      'he-p1': s.Photo1, 'he-p2': s.Photo2, 'he-p3': s.Photo3,
      'he-s1': s.Slider1, 'he-s2': s.Slider2
    };
...
async function saveHomePage() {
  const fields = {
    TempleName: 'he-name', Tagline: 'he-tagline', Timings: 'he-timings',
    Address: 'he-address', Phone: 'he-phone', Email: 'he-email',
    WhatsApp: 'he-whatsapp', MapURL: 'he-mapurl', LogoURL: 'he-logo',
    DonationUPI: 'he-upi', QRCodeURL: 'he-qr',
    Update1: 'he-upd1', Update2: 'he-upd2', Update3: 'he-upd3',
    Festival1: 'he-fes1', Festival2: 'he-fes2',
    Photo1: 'he-p1', Photo2: 'he-p2', Photo3: 'he-p3',
    Slider1: 'he-s1', Slider2: 'he-s2'
  };
  ...
  try {
    ...
    if (res.success) {
      ...
      msgEl.textContent = 'Settings saved! Reload the page to see changes.';
      msgEl.classList.remove('hidden');
      loadPublicData();
    } else {
      msgEl.className = 'alert-error';
      msgEl.textContent = res.message || 'Save failed.';
      msgEl.classList.remove('hidden');
    }
  } catch(e) {
...
// ===== MOCK RESPONSES (Demo mode before Apps Script is connected) =====
function mockResponse(payload) {
  const { action } = payload;
  if (action === 'login') {
    if (payload.username === 'admin' && payload.password === 'admin123') {
      return { success: true, username: 'admin', role: 'admin', permissions: { HomeEdit:'Yes', Finance:'Yes', Reports:'Yes', Downloads:'Yes' } };
    }
    if (payload.username === 'user1' && payload.password === 'user123') {
      return { success: true, username: 'user1', role: 'user', permissions: { HomeEdit:'No', Finance:'Yes', Reports:'Yes', Downloads:'No' } };
    }
  ...
  if (action === 'getPublicData') {
    return {
      success: true,
      settings: {
        TempleName: CONFIG.TEMPLE_NAME, Tagline: CONFIG.TEMPLE_TAGLINE,
        Timings: 'Morning 6:00 AM – 12:00 PM | Evening 4:00 PM – 8:00 PM',
        Address: '123 Temple Street, Tamil Nadu – 600001',
        Phone: '+91 98765 43210', Email: 'temple@example.com', WhatsApp: '9876543210',
        MapURL: '', LogoURL: '', DonationUPI: 'temple@upi', QRCodeURL: ''
      },
      updates: [
        { Title: 'Renovation Work', Content: 'Main hall renovation in progress. Please co-operate.', Date: '20 Jun 2026' },
        { Title: 'Annadhanam Notice', Content: 'Free food served every Sunday from 11 AM.', Date: '15 Jun 2026' },
        { Title: 'Donation Drive', Content: 'Building fund drive — contributions welcome.', Date: '10 Jun 2026' }
      ],
      festivals: [
        { Name: 'Aadi Perukku', Date: '3 Aug 2026' },
        { Name: 'Vinayaka Chaturthi', Date: '22 Aug 2026' },
        { Name: 'Navaratri', Date: '22 Sep 2026' }
      ],
      sliders: [],
      categories: ['Donation','Interest','Festival','Annadhanam','Building Fund','Temple Maintenance'],
      payments: ['Cash','UPI','Google Pay','PhonePe','Bank Transfer']
    };
  }
  if (action === 'getDashboard') {
    return {
      success: true, todayTotal: 12500, monthTotal: 87000, grandTotal: 534000,
      totalExpense: 215000,
      recent: [
        { ReceiptNo:'TR000012', Date:'2026-06-27', Name:'Rajan', Category:'Donation', Amount:1000, PaymentType:'UPI' },
        { ReceiptNo:'TR000011', Date:'2026-06-27', Name:'Sundar', Category:'Festival', Amount:500, PaymentType:'Cash' },
        { ReceiptNo:'TR000010', Date:'2026-06-26', Name:'Priya', Category:'Building Fund', Amount:5000, PaymentType:'Google Pay' }
      ]
    };
  }
  ...
  if (action === 'getTransactions') {
    const txns = [];
    for (let i = 20; i >= 1; i--) {
      txns.push({ ReceiptNo:`TR${String(i).padStart(6,'0')}`, Date:'2026-06-'+(i<10?'0'+i:i),
        Name:['Rajan','Sundar','Priya','Kumar','Meena'][i%5], Area:['Chennai','Madurai','Coimbatore'][i%3],
        Category:['Donation','Festival','Building Fund'][i%3], Amount: (i*100)+'',
        PaymentType:['Cash','UPI','Google Pay'][i%3], WhatsApp:'', Remarks:'', EnteredBy:'admin' });
    }
    return { success: true, transactions: txns };
  }
  if (action === 'getReport') {
    const data = [
      { Category:'Donation', Total:'₹45,000', Count:23 },
      { Category:'Festival', Total:'₹18,000', Count:12 },
      { Category:'Building Fund', Total:'₹24,000', Count:8 }
    ];
    return { success: true, data, columns: Object.keys(data[0]) };
  }
  if (action === 'getUsers') {
  ...
  if (action === 'getPermissions') {
    return { success: true, permissions: [
      { Username:'user1', HomeEdit:'No', Finance:'Yes', Reports:'Yes', Downloads:'No' }
    ]};
  }
  ...
  if (action === 'getPhotos') {
    return { success: true, photos: [
      { url: 'https://upload.wikimedia.org/wikipedia/commons/thumb/4/4e/Meenakshi_amman_temple.jpg/320px-Meenakshi_amman_temple.jpg', caption: 'Temple View' }
    ]};
  }
...
function togglePass(id, icon) {
  const el = document.getElementById(id);
  if (el.type === 'password') { el.type = 'text'; icon.innerHTML = '<i class="fas fa-eye-slash"></i>'; }
  else { el.type = 'password'; icon.innerHTML = '<i class="fas fa-eye"></i>'; }
}
